...
ImmutableAI.immutableCoreComponent = setImmutableCoreComponent
ImmutableAI.immutableCoreModel = setImmutableCoreModel
ImmutableAI.immutableCoreTask = setImmutableCoreTask
...
/**
 * @function setImmutableCoreComponent
 *
 * set ImmutableCoreComponent object to use
 *
 * @param {object} val
 *
 * @returns {ImmutableAI}
 */
function setImmutableCoreComponent (val) {
    // get global immutable core object
    GLOBAL.immutableCoreComponent = val
    // return class
    return ImmutableAI
}
...
/**
 * @function setImmutableCoreTask
 *
 * set ImmutableCoreTask object to use
 *
 * @param {ImmutableCoreTask} val
 *
 * @returns {ImmutableAI}
 */
function setImmutableCoreTask (val) {
    // get global immutable core object
    GLOBAL.immutableCoreTask = val
    // return class
    return ImmutableAI
}
...
function getProxy (call, property) {
    ...
    // use property name to set method name
        ...
        // if namespace is component create new component instance
        else if (call.namespace === 'component') {
            // property must be new
            assert.ok(property === 'new', `invalid method ${property} for component ${call.module}`)
            // create new component
            return newComponent(call)
        }
        // if namespace is task create new task instance
        else if (call.namespace === 'task') {
            // property must be new
            assert.ok(property === 'new', `invalid method ${property} for task ${call.module}`)
            // create new task
            return newTask(call)
        }
 ...
 */
function newComponent (call) {
    // get clone of call and reset original
    call = resetCall(call)
    // require ImmutableCoreComponent
    assert.ok(GLOBAL.immutableCoreComponent !== undefined, 'ImmutableAI configuration error: immutableCoreComponent required')
    // get component - throws error if not found
    var component = GLOBAL.immutableCoreComponent.component(call.module)
    // return function wrapper that calls new with session
    return args => {
        // make sure args is object
        if (!defined(args)) {
            args = {}
        }
        // add session to args if not defined
        if (!defined(args.session)) {
            args.session = call.session
        }
        // create new component
        return component.new(args)
    }
}
...
function newTask (call) {
    ...
    call = resetCall(call)
    // require ImmutableCoreComponent
    assert.ok(GLOBAL.immutableCoreTask !== undefined, 'ImmutableAI configuration error: immutableCoreTask required')
    // get task - throws error if not found
    var task = GLOBAL.immutableCoreTask.task(call.module)
    // return function wrapper that calls new with session
    return args => {
        // make sure args is object
        if (!defined(args)) {
